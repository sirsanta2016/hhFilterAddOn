...
function fireMouseSequence(el) {
  ...
  async function selectFactionFromShowFaction(factionName) {
    ...
    await openDropdownByField(field, true);
    await sleep(180);
  ...
  async function clearSelectedFaction() {
    const field = await waitFor(() => findFieldByLabel(SHOW_FACTION_LABEL), 3000);
    if (!field) return false;

    const currentValue = getFieldValueText(field);
    if (!currentValue || currentValue === 'All' || currentValue === 'Show faction') return true;

    await openDropdownByField(field, true);
    await sleep(180);

    const resetOption = getPanelOptions().find(opt => {
      const label = normText(getOptionLabel(opt)).toLowerCase();
      return label === 'all' || label === 'any' || label === 'none';
    });

    if (resetOption) {
      fireMouseSequence(resetOption);
      await sleep(250);
    }
  ...
  async function init() {
    ...
    const shouldShowSupport = isArtifactsPage() || isChampionsPage();
    toggleSupportButton(shouldShowSupport);
  ...
  const observer = new MutationObserver(async () => {
    if (location.href !== lastUrl) {
      ...
      toggleSupportButton(isArtifactsPage() || isChampionsPage());
